        ...
        <div class="px-12 pb-12 -mt-20 relative flex flex-col md:flex-row items-center md:items-end gap-8">
            ...
            <div class="md:pb-4 text-center md:text-left flex-1">
                <div class="flex items-center justify-center md:justify-start gap-4 mb-2">
                    <h1 class="text-4xl font-black text-slate-800 tracking-tight"><jsp:getProperty name="userBean" property="fullName" /></h1>
                    <span class="px-3 py-1 rounded-lg text-[0.6rem] font-black uppercase tracking-widest bg-amber-50 text-amber-600 border border-amber-100">Staff</span>
                </div>
                <div class="flex flex-wrap items-center justify-center md:justify-start gap-4">
                    ...
                    <span class="text-xs font-black text-slate-400 uppercase tracking-widest">Platform Admin</span>
                </div>
            </div>
            
            <div class="flex gap-4">
                <a href="#photo-section" class="bg-slate-50 text-slate-600 hover:bg-slate-100 px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all">Change Asset</a>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-7 space-y-10">
            ...
            <div class="bg-white rounded-[2.5rem] p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-100">
                <div class="flex items-center gap-4 mb-10">
                    ...
                    <h2 class="text-xl font-black text-slate-800 tracking-tight">Admin Profile</h2>
                </div>

                <form method="post" action="<%= ctx %>/admin/profile" class="space-y-8">
                    <input type="hidden" name="action" value="updateProfile" />
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="group">
                            <label class="block text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">System Identifier</label>
                        ...
                        <div class="group">
                            <label for="fullName" class="block text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Display Name</label>
                    ...
                    <div class="group">
                        <label for="phone" class="block text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Emergency Contact</label>
            ...
            <div class="bg-white rounded-[2.5rem] p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-100">
                ...
                <form method="post" action="<%= ctx %>/admin/profile" class="space-y-8">
                    ...
                    <div>
                        <label for="currentPassword" class="block text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Current Key</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label for="newPassword" class="block text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">New Key</label>
                            <input type="password" id="newPassword" name="newPassword" placeholder="••••••••" required
                                   class="w-full px-6 py-4.5 bg-slate-50 rounded-2xl focus:bg-white focus:ring-8 focus:ring-purple-500/5 focus:border-purple-500 border-2 border-transparent transition-all font-black text-slate-700 outline-none text-sm" />
                        </div>
                        <div>
                            <label for="confirm" class="block text-[0.7rem] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Confirm New Key</label>
        ...
        <div id="photo-section" class="lg:col-span-5">
            <div class="bg-white rounded-[2.5rem] p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-100 sticky top-10">
                <div class="flex items-center gap-4 mb-10">
                    ...
                    <h2 class="text-xl font-black text-slate-800 tracking-tight">Identity Image</h2>
                </div>
                ...
                <div class="flex flex-col items-center mb-12">
                    <div class="relative mb-8">
                        <% if (userBean.getProfilePhoto() != null && userBean.getProfilePhoto().length > 0) { %>
                            ...
                            <form method="post" action="<%= ctx %>/admin/profile" class="absolute -bottom-3 -right-3">
                                <input type="hidden" name="action" value="deletePhoto" />
                                <button type="submit" onclick="return confirm('Wipe identity photo?')" 
                ...
                <form method="post" action="<%= ctx %>/admin/profile" enctype="multipart/form-data" class="space-y-6">
                    <input type="hidden" name="action" value="uploadPhoto" />
                    <label for="adminPhoto" class="block group cursor-pointer group">
                        <div class="flex flex-col items-center gap-3 px-8 py-12 border-4 border-dashed border-slate-100 rounded-[2.5rem] group-hover:border-brand/30 group-hover:bg-brand/5 transition-all text-center bg-slate-50/50">
                            ...
                            <div class="mt-2 text-center">
                                <p class="text-sm font-black text-slate-700">Push New Image</p>
                                <p class="text-[0.7rem] font-bold text-slate-400 uppercase tracking-widest mt-1">Universal Image Format</p>
                            </div>
                        </div>
                        <input type="file" id="adminPhoto" name="photo" accept=".png,.jpg,.jpeg,.gif,.webp" required class="hidden" />
                    </label>
                    
                    <div id="adminFileNameDisplay" class="hidden px-5 py-3 rounded-xl bg-slate-50 border border-slate-100 text-center">
                        <span class="text-xs font-black text-slate-500 uppercase tracking-widest truncate block" id="adminFileNameText">No file chosen</span>
                    </div>
...
<script>
document.getElementById('adminPhoto').addEventListener('change', function() {
    const display = document.getElementById('adminFileNameDisplay');
    const text = document.getElementById('adminFileNameText');
